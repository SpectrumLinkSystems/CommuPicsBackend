...
from apps.child.services.child_service import (
    update_autism_level,
    generar_qr,
)
...
class ChildViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=["get"], url_path="(?P<child_pk>[^/.]+)/collections")
    def get_collections_by_child_id(self, request, child_pk):
        try:
            # Obtener todas las colecciones del niño
            collections = Collection.objects.filter(child_id=child_pk)
...
class ParentChildViewSet(GenericViewSet, CreateModelMixin, ListModelMixin):
...
class TherapistChildViewSet(GenericViewSet, ListModelMixin):
    queryset = Child.objects.all()
    serializer_class = ChildSerializer
