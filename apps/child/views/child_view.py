...
from apps.child.services.child_service import (create_child_for_parent,
...
class ChildViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, parent_pk=None):
        child = create_child_for_parent(
            parent_pk, request.data, request.FILES.get("avatar")
        )
        if child:
            serializer = ChildSerializer(child)
            return Response(
                {"message": "Child created", "child": serializer.data},
                status=status.HTTP_201_CREATED,
            )
        return Response(
            {"error": "Failed to create child or parent not found"},
    ...
    def list(self, request, parent_pk=None):
        ...
        return Response(
            {"error": "Parent not found or no children"},
    ...
    def retrieve(self, request, parent_pk=None, pk=None):
        ...
        if child:
            ...
            return Response(serializer.data)
        return Response(
            {"error": "Child or Parent not found"}, status=status.HTTP_404_NOT_FOUND
        )

    def update(self, request, parent_pk=None, pk=None):
        # Handle file uploads if present
        child = update_child_for_parent(
            parent_pk, pk, request.data, request.FILES.get("avatar")
        )
        if child:
            serializer = ChildSerializer(child)
    ...
    def destroy(self, request, parent_pk=None, pk=None):
        if delete_child_for_parent(parent_pk, pk):
            return Response(
                {"message": "Child deleted"}, status=status.HTTP_204_NO_CONTENT
            )
        return Response(
            {"error": "Child or Parent not found"}, status=status.HTTP_404_NOT_FOUND
        )
