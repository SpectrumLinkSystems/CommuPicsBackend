...
class RecognitionView(APIView):
    ...
    async def post(self, request):
        ...
        collections = await sync_to_async(
            lambda: list(Collection.objects.filter(child_id=child_id).values())
        )()
        recognition_response = await self.image_recognition_service.recognize_image(
